...
public class ProcessTestWorkflowMonitoringExecutionV3 {

	@Rule 
	public ActivitiRule activitiRule = new ActivitiRule("activiti.cfg-mem.xml");

	@Test
	@Deployment(resources={"WorkflowMonitoringExecutionV3.bpmn"})
...
//		System.out.println("taskDefinition einai iso me : " + taskDefinition);
		...
		//  HERE I GET THE FLOWS OF TASKS THAT WHERE TAKEN
		
//		 List<HistoricTaskInstance> historicTaskInstances = activitiRule.getHistoryService().createHistoricTaskInstanceQuery().list();
		List<HistoricActivityInstance> historicTaskInstances = activitiRule.getHistoryService()
				 .createHistoricActivityInstanceQuery()
				 .orderByHistoricActivityInstanceEndTime()
				 .asc()
				 .list();
		 
		 System.out.println("To size einai iso me : " + historicTaskInstances.size());
		 for (HistoricActivityInstance ht : historicTaskInstances) {
			  System.out.println("The activity type is: " + ht.getActivityType());
		      assertNotNull(ht.getStartTime());
		      System.out.println("StartTime for taskId " + ht.getProcessDefinitionId() + " and startTime " + ht.getStartTime() + " and " + ht.getActivityName());
		      assertNotNull(ht.getEndTime());
		      System.out.println("EndTime for taskId " + ht.getProcessDefinitionId()+ " and endTime " + ht.getEndTime() + " and " + ht.getActivityName());
		    }
		 ...
		 // HERE I GET THE FLOWS BY THE NUMBER THEY WHERE GOTTERN
		 List<HistoricVariableInstance> historyVariables2 =  activitiRule.getHistoryService()
					.createHistoricVariableInstanceQuery()
					.orderByVariableName()
					.asc()
					.list();
		 for (int i=0; i<historyVariables2.size(); i++){
			 HistoricVariableInstance hsv = 
					((HistoricVariableInstance)historyVariables2.get(i));
			 if(hsv.getVariableName().equals("sequenceFlows")){
			 System.out.println("To variable einai iso me : " + hsv.toString());
			 ArrayList<String> sequenceFlowsH = (ArrayList<String>) hsv.getValue();
			 System.out.println("To List me idFlows exei size iso me : " + sequenceFlowsH.size());
			 for(int j=0; j<sequenceFlowsH.size(); j++){
				 System.out.println("To " + j+1 +"o htan iso me " + sequenceFlowsH.get(j));
			 }
			 }
		 }
		System.out.println("To size toy historyVariables2 einai iso me : " + historyVariables2.size());
